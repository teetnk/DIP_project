...
# ✅ ฟังก์ชัน Retrain โมเดล
def retrain_model(is_new_class):
    global model, CLASS_NAMES

    print(f"🚀 เริ่ม retrain โมเดล (ใหม่: {is_new_class})...")

    # ✅ 1. ตรวจสอบและลบไฟล์ที่ไม่ใช่ภาพก่อนใช้ ImageFolder()
    for folder in os.listdir(TRAINING_FOLDER):
        folder_path = os.path.join(TRAINING_FOLDER, folder)

        if os.path.isdir(folder_path):  # ✅ เช็คว่าเป็นโฟลเดอร์จริงๆ
            for file in os.listdir(folder_path):
                file_path = os.path.join(folder_path, file)

                # ✅ ถ้าเป็นไฟล์และไม่ใช่ไฟล์รูปภาพ ให้ลบทิ้ง
                if os.path.isfile(file_path) and not file.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp')):
                    print(f"⚠️ ลบไฟล์ที่ไม่รองรับ: {file_path}")
                    os.remove(file_path)

                # ✅ ถ้าเป็นโฟลเดอร์ที่ผิดพลาด ให้ลบโฟลเดอร์
                if os.path.isdir(file_path):
                    print(f"⚠️ ลบโฟลเดอร์ที่ไม่รองรับ: {file_path}")
                    shutil.rmtree(file_path, ignore_errors=True)

    time.sleep(1)  # ✅ รอให้ไฟล์ถูกลบจริงๆ

    # ✅ 2. ตรวจสอบว่าแต่ละโฟลเดอร์มีไฟล์รูปภาพอย่างน้อย 1 รูป
    valid_folders = [f for f in os.listdir(TRAINING_FOLDER) if os.path.isdir(os.path.join(TRAINING_FOLDER, f))]
    valid_data = False

    for folder in valid_folders:
        folder_path = os.path.join(TRAINING_FOLDER, folder)
        image_files = [f for f in os.listdir(folder_path) if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.tiff', '.webp'))]
        
        if len(image_files) > 0:
            valid_data = True  # ✅ พบไฟล์ภาพแล้ว
        else:
            print(f"⚠️ โฟลเดอร์ {folder} ไม่มีไฟล์ภาพที่ใช้ได้ ข้ามการเทรน")
    
    if not valid_data:
        print("❌ ไม่มีข้อมูลภาพที่ใช้ได้สำหรับการฝึกโมเดล!")
        return
    ...
    try:
        ...
        print(f"📸 จำนวนภาพที่ใช้เทรน: {len(train_dataset)} ภาพ")
    except Exception as e:
        print(f"❌ ERROR ในการโหลด datasets.ImageFolder: {e}")
        return

    # ✅ 4. สร้างโมเดลใหม่และโหลด State
